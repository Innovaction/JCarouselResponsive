...
namespace DotNetNuke.Modules.JCarouselResponsive
{
    ...
    /// 
    /// Because the control inherits from JCarouselResponsiveSettingsBase you have access to any custom properties
    /// defined there, as well as properties from DNN such as PortalId, ModuleId, TabId, UserId and many more.
    /// </summary>
    /// -----------------------------------------------------------------------------
    public partial class Settings : JCarouselResponsiveSettingsBase
    {
        ...
        public override void LoadSettings()
        {
            try
            {
                ...
                {
                    //Check if stylesheet exists, else is first time module loads
                    if (Settings.Contains("StyleSheet"))
                    {
                        TextBoxStyleSheet.Text = Settings["StyleSheet"].ToString();
                    }
                    else
                    {
                        Settings["StyleSheet"] = DefaultStyleSheet.StyleSheet;
                        TextBoxStyleSheet.Text = Settings["StyleSheet"].ToString();
                    }
        ...
        public string GenerateStyleSheetWithModuleID(string styleSheet)
        {
            MatchCollection mt = Regex.Matches(styleSheet, @"[^}]?([^{]*{[^}]*})", RegexOptions.Multiline);
